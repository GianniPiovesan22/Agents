...
import { getPendingReminders, markReminderSent, getRecurringReminders, updateReminderNextFire, getUpcomingHighImpactEvents, markForexEventNotified, saveForexEvents, getStaleLeads, getRecentContentTypes } from './database/index.js';
import { parseForexCalendar } from './tools/forex_factory.js';
    ...
    try {
        ...
        cron.schedule('0 7 * * *', async () => {
            try {
                ...
                const parsed = parseForexCalendar(markdown);
                if (parsed.length > 0) {
